...
-- Esquema: dgt_pdm
-- ==========================================================
-- DROP SCHEMA IF EXISTS dgt_pdm; -- DROP SCHEMA IF EXISTS dgt_pdm CASCADE;
...
-- Table: codigo_ine
-- ==========================================================
-- DROP TABLE IF EXISTS dgt_pdm.codigo_ine;
CREATE TABLE IF NOT EXISTS dgt_pdm.codigo_ine (
...
-- ==========================================================
-- DROP TABLE IF EXISTS dgt_pdm.atribuir_municipio;
CREATE TABLE IF NOT EXISTS dgt_pdm.atribuir_municipio (
    id   SERIAL PRIMARY KEY CHECK (id = 1),
    dtcc TEXT UNIQUE,
    FOREIGN KEY (dtcc) REFERENCES dgt_pdm.codigo_ine (dtcc)
);
...
-- Table: objeto_planta
-- ==========================================================
-- DROP TABLE IF EXISTS dgt_pdm.objeto_planta;
CREATE TABLE IF NOT EXISTS dgt_pdm.objeto_planta (
    id     SERIAL PRIMARY KEY,
...
-- Table: catalogo
-- ==========================================================
-- DROP TABLE IF EXISTS dgt_pdm.catalogo;
CREATE TABLE IF NOT EXISTS dgt_pdm.catalogo (
    id                SERIAL PRIMARY KEY,
    ...
    FOREIGN KEY (planta) REFERENCES dgt_pdm.objeto_planta (planta) 
);
...
CREATE TABLE IF NOT EXISTS dgt_pdm.ato_especifico (
    id                SERIAL PRIMARY KEY,
    id_ato_especifico SERIAL UNIQUE,
    serie_dr          TEXT,
    numero_dr         TEXT,
    tipo_ato          TEXT,
    ...
    dtcc              TEXT,
    FOREIGN KEY (dtcc) REFERENCES dgt_pdm.atribuir_municipio (dtcc)
        ON DELETE CASCADE
        ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS dgt_pdm.objeto_ponto (
    ...
    FOREIGN KEY (planta, objeto_codigo) REFERENCES dgt_pdm.catalogo (planta, objeto_codigo),
    FOREIGN KEY (id_ato_especifico) REFERENCES dgt_pdm.ato_especifico (id_ato_especifico),
    FOREIGN KEY (dtcc) REFERENCES dgt_pdm.atribuir_municipio (dtcc)
        ON DELETE CASCADE 
        ON UPDATE CASCADE,
    FOREIGN KEY (planta) REFERENCES dgt_pdm.objeto_planta (planta),
...
-- View: catalogo_cond
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_cond;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_cond AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
...
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_cond_pt;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_cond_pt AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
     ...
     WHERE t.planta = 'Condicionantes' AND
           lower(unaccent(t.geometria)) LIKE lower(unaccent('%ponto%'));


-- ==========================================================
-- View: catalogo_cond_ln
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_cond_ln;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_cond_ln AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
      ...
      FROM dgt_pdm.catalogo t
     WHERE t.planta = 'Condicionantes' AND
           lower(unaccent(t.geometria)) LIKE lower(unaccent('%linha%'));


-- ==========================================================
-- View: catalogo_cond_pl
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_cond_pl;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_cond_pl AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
      ...
      FROM dgt_pdm.catalogo t
     WHERE t.planta = 'Condicionantes' AND
           lower(unaccent(t.geometria)) LIKE lower(unaccent('%polígono%'));


-- ==========================================================
-- View: catalogo_ord
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_ord;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_ord AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
...
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_ord_pt;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_ord_pt AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
...
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_ord_ln;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_ord_ln AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
...
-- ==========================================================
-- DROP VIEW IF EXISTS dgt_pdm.catalogo_ord_pl;
CREATE OR REPLACE VIEW dgt_pdm.catalogo_ord_pl AS
    SELECT t.id,
           t.objeto_codigo,
           t.planta,
           t.tema,
...
-- ==========================================================
-- DROP FUNCTION IF EXISTS dgt_pdm.calc_area_m2_geom();
CREATE OR REPLACE FUNCTION dgt_pdm.calc_area_m2_geom()
RETURNS TRIGGER AS $$
BEGIN
...
-- ==========================================================
-- DROP FUNCTION IF EXISTS dgt_pdm.calc_comp_m_geom();
CREATE OR REPLACE FUNCTION dgt_pdm.calc_comp_m_geom()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.geom IS NOT NULL THEN
...
-- Trigger: gerir_uuid
-- ==========================================================
-- DROP FUNCTION IF EXISTS dgt_pdm.gerir_uuid();
CREATE OR REPLACE FUNCTION dgt_pdm.gerir_uuid()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.uuid IS NULL 
...
-- Trigger: gerir_uuid
-- ==========================================================
-- DROP FUNCTION IF EXISTS dgt_pdm.protect_uuid();
CREATE OR REPLACE FUNCTION dgt_pdm.protect_uuid() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
...
-- Table: objeto_ponto
-- ==========================================================
-- Trigger: objeto_ponto_calc_area_m2_geom
-- ==========================================================
-- DROP TRIGGER IF EXISTS objeto_ponto_calc_area_m2_geom ON dgt_pdm.objeto_ponto;
CREATE OR REPLACE TRIGGER objeto_ponto_calc_area_m2_geom
    BEFORE INSERT OR UPDATE OF geom
    ON dgt_pdm.objeto_ponto
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.calc_area_m2_geom();

-- Trigger: objeto_ponto_gerir_uuid
-- ==========================================================
-- DROP TRIGGER IF EXISTS objeto_ponto_gerir_uuid ON dgt_pdm.objeto_ponto;
CREATE TRIGGER objeto_ponto_gerir_uuid
    BEFORE INSERT OR UPDATE ON dgt_pdm.objeto_ponto
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.gerir_uuid();

-- Trigger: objeto_ponto_protect_uuid
-- ==========================================================
--DROP TRIGGER IF EXISTS objeto_ponto_protect_uuid ON dgt_pdm.objeto_ponto;
CREATE TRIGGER objeto_ponto_protect_uuid
    BEFORE UPDATE OF uuid ON dgt_pdm.objeto_ponto
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.protect_uuid();


-- ==========================================================
-- Table: objeto_linha
-- ==========================================================
-- Trigger: objeto_linha_calc_comp_m_geom
-- ==========================================================
-- DROP TRIGGER IF EXISTS objeto_linha_calc_comp_m_geom ON dgt_pdm.objeto_linha;
CREATE TRIGGER objeto_linha_calc_comp_m_geom
    BEFORE INSERT OR UPDATE 
    OF geom ON dgt_pdm.objeto_linha
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.calc_comp_m_geom();

-- Trigger: objeto_linha_gerir_uuid
-- ==========================================================
-- DROP TRIGGER IF EXISTS objeto_linha_gerir_uuid ON dgt_pdm.objeto_linha;
CREATE TRIGGER objeto_linha_gerir_uuid
    BEFORE INSERT OR UPDATE ON dgt_pdm.objeto_linha
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.gerir_uuid();

-- Trigger: objeto_linha_protect_uuid
-- ==========================================================
--DROP TRIGGER IF EXISTS objeto_linha_protect_uuid ON dgt_pdm.objeto_linha;
CREATE TRIGGER objeto_linha_protect_uuid
    BEFORE UPDATE OF uuid ON dgt_pdm.objeto_linha
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.protect_uuid();
...
-- Table: objeto_poligono
-- ==========================================================
-- Trigger: objeto_poligono_calc_area_m2_geom - Table: objeto_poligono
-- ==========================================================
-- DROP TRIGGER IF EXISTS objeto_poligono_calc_area_m2_geom ON dgt_pdm.objeto_poligono;
CREATE TRIGGER objeto_poligono_calc_area_m2_geom
    BEFORE INSERT OR UPDATE OF geom 
    ON dgt_pdm.objeto_poligono
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.calc_area_m2_geom();

-- Trigger: objeto_poligono_gerir_uuid
-- ==========================================================
-- DROP TRIGGER IF EXISTS objeto_poligono_gerir_uuid ON dgt_pdm.objeto_poligono;
CREATE TRIGGER objeto_poligono_gerir_uuid
    BEFORE INSERT OR UPDATE ON dgt_pdm.objeto_poligono
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.gerir_uuid();

-- Trigger: objeto_poligono_protect_uuid
-- ==========================================================
--DROP TRIGGER IF EXISTS objeto_poligono_protect_uuid ON dgt_pdm.objeto_poligono;
CREATE TRIGGER objeto_poligono_protect_uuid
    BEFORE UPDATE OF uuid ON dgt_pdm.objeto_poligono
    FOR EACH ROW
    EXECUTE FUNCTION dgt_pdm.protect_uuid();
...
-- Table: objeto_planta
-- ==========================================================
INSERT INTO dgt_pdm.objeto_planta 
    (planta) 
VALUES 
    ('Ordenamento'),
    ('Condicionantes');
...
-- Tabela: catalogo
-- ==========================================================
INSERT INTO dgt_pdm.catalogo
     (id, objeto_codigo, planta, tema, subtema, objeto_designacao, etiqueta, geometria, observacoes)
 VALUES 
...
-- Table: codigo_ine
-- ==========================================================
INSERT INTO dgt_pdm.codigo_ine
     (id, municipio, dtcc)
 VALUES 
     (1, 'Abrantes', '1401'),
     (2, 'Águeda', '0101'),
...
-- Table: atribuir_municipio
-- ==========================================================
INSERT INTO dgt_pdm.atribuir_municipio 
     (dtcc)
 VALUES 
     ('1401');
...
WITH meta_dados_gpkg_contents (tipo, table_name, data_type, identifier, description, last_change, min_x, min_y, max_x, max_y, srs_id) AS (
 VALUES
     ('TABLE', 'atribuir_municipio', 'attributes', 'atribuir_municipio', 'Identifica o município de trabalho do utilizador. Esta tabela contém um único registo e funciona como elemento de configuração, centralizando a identificação do território a que os dados se referem através do código DTCC. O respetivo código atua como chave estrangeira nas tabelas gráficas, garantindo a integridade referencial. Qualquer alteração ao registo é automaticamente refletida nas tabelas dependentes, assegurando a associação de todos os dados ao mesmo município.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, NULL),
     ('TABLE', 'catalogo', 'attributes', 'catalogo', 'Identifica os objetos que podem existir na base de dados, a natureza geométrica que cada objeto pode assumir e a sua organização em cada planta do plano.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, NULL),
     ('TABLE', 'codigo_ine', 'attributes', 'codigo_ine', 'Lista os municípios e respetivos códigos da divisão administrativa do Instituto Nacional de Estatística (código DTCC).', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, NULL),
     ('TABLE', 'objeto_linha', 'features', 'objeto_linha', 'Contém os objetos com geometria do tipo linear.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, 3763),
     ('TABLE', 'objeto_planta', 'attributes', 'objeto_planta', 'Garante a integridade relacional das geometrias (tabelas gráficas) e alfanumérica (tabela catalogo) com as plantas do PDM (Ordenamento e Condicionantes), utilizando chaves compostas e restrições condicionais baseadas no tipo de planta.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, NULL),
     ('TABLE', 'objeto_poligono', 'features', 'objeto_poligono', 'Contém os objetos com geometria do tipo poligonal.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, 3763),
     ('TABLE', 'objeto_ponto', 'features', 'objeto_ponto', 'Contém os objetos com geometria do tipo pontual.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, 3763),
     ('VIEW', 'catalogo_cond', 'attributes', 'catalogo_cond', 'catalogo_cond', '2026-08-07T06:58:47.678Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_cond_ln', 'attributes', 'catalogo_cond_ln', 'catalogo_cond_ln', '2026-08-07T06:58:47.680Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_cond_pl', 'attributes', 'catalogo_cond_pl', 'catalogo_cond_pl', '2026-08-07T06:58:47.682Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_cond_pt', 'attributes', 'catalogo_cond_pt', 'catalogo_cond_pt', '2026-08-07T06:58:47.684Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_ord', 'attributes', 'catalogo_ord', 'catalogo_ord', '2026-08-07T06:58:47.686Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_ord_ln', 'attributes', 'catalogo_ord_ln', 'catalogo_ord_ln', '2026-08-07T06:58:47.688Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_ord_pl', 'attributes', 'catalogo_ord_pl', 'catalogo_ord_pl', '2026-08-07T06:58:47.690Z', NULL, NULL, NULL, NULL, NULL),
     ('VIEW', 'catalogo_ord_pt', 'attributes', 'catalogo_ord_pt', 'catalogo_ord_pt', '2026-08-07T06:58:47.692Z', NULL, NULL, NULL, NULL, NULL),
     ('TABLE', 'ato_especifico', 'attributes', 'ato_especifico', 'Tabela de preenchimento exclusivo para os objetos da Planta de Condicionantes que tenham associada uma servidão ou restrição de utilidade pública constituída por ato específico.', '2026-08-17T13:27:34.380Z', NULL, NULL, NULL, NULL, NULL)
)
SELECT CONCAT('COMMENT ON ', tipo, ' dgt_pdm.', table_name, ' IS ', quote_literal(description), ';') AS ddl, * FROM meta_dados_gpkg_contents
...
WITH meta_dados_gpkg_data_columns (table_name, column_name, name, title, description, mime_type, constraint_name) AS (
 VALUES
     ...
     ('atribuir_municipio', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela. Chave primária.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ('ato_especifico', 'dtcc', 'DTCC', NULL, 'Descrição: Código DTCC que identifica o município.; Alias: DTCC; Tipo de data: TEXT', NULL, NULL),
     ('ato_especifico', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela. Chave primária da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ('objeto_planta', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela. Chave primária.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('objeto_ponto', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela. Chave primária.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('objeto_linha', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela. Chave primária.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('objeto_poligono', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela. Chave primária.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_ord', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_cond', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_cond_pt', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_cond_ln', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_cond_pl', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_ord_pt', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_ord_ln', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
     ...
     ('catalogo_ord_pl', 'id', 'id', NULL, 'Descrição: Campo de numeração automática e crescente, que identifica univocamente cada linha da tabela.; Alias: id; Tipo de data: INTEGER', NULL, NULL),
...
SELECT CONCAT('COMMENT ON COLUMN ', 'dgt_pdm.', table_name, '.', column_name, ' IS ', quote_literal(REGEXP_REPLACE(description, '[;,]?\s*Tipo de data:.*', '', 'i')), ';') AS ddl, * FROM meta_dados_gpkg_data_columns
